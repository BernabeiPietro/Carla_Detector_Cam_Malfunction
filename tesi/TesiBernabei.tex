...
quale sono applicati. Per un’immagine RGB, il primo filtro convoluzionale
F*F*d potrà avere dimensione  5*5*3 o 7*7*3. Il numero di possibili
allineamenti tra filtro e immagine definisce altezza e larghezza della successiva feature
map.
Dallo strato nascosto q1, il filtro produce uno nuovo strato nascosto q2
Lq+1 = Lq - Fq + 1
Bq+1 = Bq - Fq + 1
...
nell’immagine.
Una delle proprietà della convoluzione è la seguente. Una convoluzione sullo strato
...
\textbf{Padding} L’operazione di convoluzione comporta una contrazione dello strato q rispetto a q + 1 ed una conseguente perdita di informazioni. Il problema può essere
...
Strides Un filtro convoluzionale computa il prodotto scalare in ogni singola posizione dello strato di input ma è altresì possibile limitare la computazione ad un numero
...
valore del bias è definito tramite retropropagazione in fase di addestramento. \cite{aggarwal2018neural}
...
\subparagraph{CARLA}
CARlA è un software open source di simulazione per la ricerca di guida autonoma.
Il simulatore CARLA è costituito da un'architettura client-server scalabile.
Il server è responsabile di tutto ciò che riguarda la simulazione stessa: rendering del sensore, calcolo della fisica, aggiornamenti sullo stato del mondo e sui suoi attori e molto altro. Poiché mira a risultati realistici, la soluzione migliore sarebbe eseguire il server con una GPU dedicata, soprattutto quando si ha a che fare con l'apprendimento automatico.
Il lato client consiste in una somma di moduli client che controllano la logica degli attori sulla scena e impostano le condizioni del mondo. 
